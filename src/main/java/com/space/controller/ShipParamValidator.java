...
public class ShipParamValidator {
   ...
   // private static final int CURRENT_YEAR = 3019;
    ...
    public static void updateRating(Ship ship) {
        ...
        double k = ship.isUsed() ? USED_SHIP_COEFFICIENT : NEW_SHIP_COEFFICIENT; //коэффициент, который равен 1 для нового корабля и 0,5 для использованного;
        double v = ship.getSpeed(); //скорость корабля;
        double result = (80d * v * k)  / (MAX_YEAR - calendar.get(Calendar.YEAR) + 1);
        result = Math.round(result * 100d) / 100d; //по задче нужно использывать Math
